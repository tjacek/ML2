-module(apriori_combremot).
% combinations produced on the remote
%%
%% Include files
%%
...
%	io:format("My Meta ~p\n", [MetaData]),
	...
	receive 
		...
		{Pid, {SetsToCheck, Id}} ->
			CombsRepeat = 
				lists:foldl(fun (L, Acu) -> L ++ Acu end, [], [ combinations([Candidate], CandTail, length(Candidate) + 1, []) 
				|| {Candidate, CandTail} <- SetsToCheck ]),
			Combinations = gb_sets:to_list(gb_sets:from_list(CombsRepeat)),
			CDict = count(Combinations, MetaData, []),
...
apriori_first(DictData, Workers, DataWorkers, ItemsetWorkers, MinSup, MinConf, Result) ->
	...
	apriori_steps(2, dict:to_list(PrunedLk), Workers, DataWorkers, ItemsetWorkers, MinSup, MinConf, dict:merge(fun (K, V1, V2) -> ?LOG("dicte merge error ~p, ~p, ~p \n",[K, V1, V2]) end, PrunedLk, Result)).
...
apriori_steps(Step, PrunedList, Workers, DataWorkers, ItemsetWorkers, MinSup, MinConf, Result) ->
	...
	apriori_steps(Step+1, dict:to_list(PrunedLk), Workers, DataWorkers, ItemsetWorkers, MinSup, MinConf, dict:merge(fun (K, V1, V2) -> ?LOG("dict merge error ~p, ~p, ~p \n",[K, V1, V2]) end, PrunedLk, Result)).
	% if empty -> finish
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
make_sets(PrunedList, ItemsetWorkers) ->
	DickList = lists:sort(fun ({S1, V1}, {S2, V2}) -> V1 =< V2 end, PrunedList),
	case DickList of
		[] -> [];
		_ ->
...
%%%
balance([{Set, V}], Sum, ItemsetWorkers) ->
	lists:map(fun ({S, V}) -> gb_sets:to_list(gb_sets:from_list(S)) end, ItemsetWorkers);
balance([{Set, V}|DickList], Sum, ItemsetWorkers) ->
	Estimate = (Sum - V) * V,
	ToReplace = get_with_lowest_todo(ItemsetWorkers),
	balance(DickList, Sum-V, add_estimate(ToReplace, Estimate, Set, DickList, ItemsetWorkers)).
...
add_estimate(ToReplace, Estimate, Set, DickList, ItemsetWorkers) ->
	add_estimate(ToReplace, Estimate, 1, Set, lists:map(fun ({S, V}) -> S end, DickList), ItemsetWorkers).

add_estimate(ToReplace, Estimate, ToReplace, Set, DickList, [{S, V}|ItemsetWorkers]) ->
	[ {[{Set, DickList}|S], V+Estimate}|ItemsetWorkers];
